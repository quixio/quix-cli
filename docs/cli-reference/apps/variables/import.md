...
- **Existing Variables**: If a variable from the `.env` file matches an existing variable in the `app.yaml` file, its `defaultValue` is replaced by the value from the `.env` file. This command always replaces — there is no option to import new variables while leaving existing ones alone. Values under the `### Secrets ###` section and secret members of a variable group are the exception: they are never overwritten, because their `defaultValue` holds a key that points at the value rather than the value itself.
