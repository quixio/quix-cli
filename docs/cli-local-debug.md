...
=== "Local Pipeline Broker"
    ...
    For more details on the `local` command, refer to the [sdk broker documentation](./Reference/sdk/broker/local.md).
...
3. **SDK broker configuration**:
   
    Depending on your setup, additional variables may be included in the `.env` file:

    === "SDK broker configuration"

        If using a SDK broker configuration, the following variable will be included:
